...
def save_floor_mesh(savepath, filename, *args):
    fid = open(savepath+'/'+str(filename)+'.obj', "w")
    v_id = 1
    vt_id = 1
    for mesh in args:
...
def vedo_transform_v(mesh, instance):
    pts = mesh.points()

    pos = instance.pos
    if hasattr(instance, 'rotscale'):
        pts = np.dot(pts, np.array(instance.rotscale))
    else:
        rot = instance.rot
        scale = instance.scale
        pts = pts.astype(np.float64) * scale
        rotMatrix = quaternion_to_matrix(rot)
        R = np.array(rotMatrix)
        ...
        pts = np.transpose(pts)
    pts = pts + pos
    mesh = mesh.points(pts)
    return mesh


def vedo_transform_mesh(mesh, pos, rot, scale):
    pts = mesh.points()
    pts = pts.astype(np.float64) * scale
    rotMatrix = quaternion_to_matrix(rot)
    R = np.array(rotMatrix)
