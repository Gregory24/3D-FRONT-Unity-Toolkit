import json
import numpy as np
from utils import read_obj, save_obj, read_mesh_attr, transform_v, save_mesh, save_floor_mesh
import os
...
class Room():
    ...
    def output_floor_meshes(self, save_folder, filename, select_mesh_type):
        for constructid, all_mesh in self.meshes.items():
            output = []
            for mesh in all_mesh:
                if len(select_mesh_type) > 0:
            ...
            if len(output) == 0:
                continue
            save_floor_mesh(save_folder, filename, *output)
...
def read_json(jsonfile, future_path):
