...
public class FileReader
{
	...
	public static ObjectFile ReadObjectFile(string path)
	{
		...
		foreach (string line in lines)
		{
			...
			string[] new_token = new string[token.Length];
			int idx = 0;
			...
			switch (new_token[0])
			{
				case ("o"):
					...
					break;
				case ("mtllib"):
					obj.mtllib = new_token[1];
					break;
				case ("usemtl"):
					obj.usemtl.Add(new_token[1]);
					obj.f.Add(new List<int[]>());
					break;
				case ("v"):
					obj.v.Add(new Vector3(
						float.Parse(new_token[1]),
						float.Parse(new_token[2]),
						float.Parse(new_token[3])));
					break;
				case ("vn"):
				...
				case ("f"):
					for (int i = 1; i < 4; i += 1)
					{
						int[] triplet = Array.ConvertAll(new_token[i].Split('/'), x => {
